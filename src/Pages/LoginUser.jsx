...
const Form = () => {
    ...
    const [msg, setMsg] = useState("Invalid User!");
    var phoneNumber = "";
    let phonenum;
    ...
    function handleVerify() {
        if (num !== null) {
            ...
            if (isLoggedIn) {
                ...
                // alert("Already Logged in! Wait for a moment while we log you out from previous session");
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
                    text: `Already Logged in with ${phonenum}! Wait for a moment while we log you out from previous session`,
                });
                setMsg("Logged out! Please retry");
                const auth = getAuth();
                signOut(auth).then(() => {
            ...
            if (!isLoggedIn) {
                axios.get(`https://10.1.2.201/api/facedetection/users/${phonenum}`).then(res => {
                    // console.log(res);
    ...
    const onSubmitOTP = (e) => {
        ...
        window.confirmationResult.confirm(code).then((result) => {
            ...
            navigate("/homeUser", { replace: true, state: { phoneNo: num } });
    ...
    return (
        <div className="LoginBody">
            <Header />
            <div style={{ display: "flex", flexDirection: "row", justifyContent: "flex-end" }}><Button variant="contained" onClick={() => {

                navigate("/loginAdmin");

            }} style={{ marginTop: "10px", marginRight: "15px" }} endIcon={<AdminPanelSettingsIcon />}>
                    ...
                    <div className="col-1">
                        ...
                        <div className='phonediv' style={{ display: "flex", alignItems: "baseline", width: "85%", margin: "15px" }}>
                            ...
                            <div className='verify'>
                                <Button className='phonebutton' variant='outlined' style={{ marginLeft: "10%", height: "40px" }} size='small' onClick={handleVerify}>Verify</Button>
                            </div>
                        </div>
                        {valid ? (
                            <div>
                            ...
                            checked ? (<div><h2 style={{ opacity: "0.2", display: "flex", marginLeft: "15%" }} id='message'>{msg}</h2></div>) : ("")
                        )}
                        <p>Don't have an account? <Link to="/registerUser">RegisterUser</Link></p>
                    </div>
                    <div className="col-2">
